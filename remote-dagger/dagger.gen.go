...
func (r *RemoteDagger) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		SSHClient *SSHClient
	}
	...
	r.SSHClient = concrete.SSHClient
	return nil
}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "RemoteDagger":
		switch fnName {
		case "SetupDagger":
			var parent RemoteDagger
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return nil, (*RemoteDagger).SetupDagger(&parent, ctx)
		case "RemoteCall":
			var parent RemoteDagger
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*RemoteDagger).RemoteCall(&parent, ctx, module, args)
		case "":
			var parent RemoteDagger
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("RemoteDagger").
					WithFunction(
						dag.Function("SetupDagger",
							dag.TypeDef().WithKind(VoidKind).WithOptional(true)).
							WithDescription("example usage: \"dagger call --destination USER@HOST --identity-file file:${HOME}/.ssh/id_ed25519 install\"")).
					WithFunction(
						dag.Function("RemoteCall",
							dag.TypeDef().WithObject("Container")).
							WithDescription("example usage: \"dagger call --destination USER@HOST --identity-file file:${HOME}/.ssh/id_ed25519 remote-call ... stdout\"").
							WithArg("module", dag.TypeDef().WithKind(StringKind)).
							WithArg("args", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)))).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("RemoteDagger")).
							WithArg("destination", dag.TypeDef().WithKind(StringKind)).
							WithArg("identityFile", dag.TypeDef().WithObject("Secret")))), nil
	default:
